...
class Solution39Test {
    ...
    void testOrder() {
        ...
        for (Map.Entry<String, String> entry : employeeData.get(0).entrySet()) {
            actualOrder.add(entry.getKey());
        }

        List<String> expectedOrder = new ArrayList<>();
        expectedOrder.add("Jackson, Jacquelyn");
        expectedOrder.add("Jacobson, Jake");
        expectedOrder.add("Johnson, John");
        expectedOrder.add("Michaelson, Michaela");
        expectedOrder.add("Webber, Sally");
        expectedOrder.add("Xiong, Tou");
